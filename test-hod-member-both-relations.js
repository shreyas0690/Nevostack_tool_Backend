...
async function testHodToMemberBothRelations() {
  try {
    console.log('🧪 Testing HOD to Member (Both Manager & HOD Relations)...');
    ...
    // Find existing HOD in new department
    ...
    if (existingHod) {
      console.log('📊 Existing HOD in new department:', existingHod.name, existingHod._id);
    } else {
    ...
    // Find existing HOD again (for the logic)
    const existingHodForLogic = await User.findOne({
      role: 'department_head',
    ...
    if (updateData.managerId) {
      ...
      if (existingHodForLogic && existingHodForLogic._id.toString() !== updateData.managerId) {
        console.log('📊 Adding to existing HOD as well since different from specified manager');

        if (!existingHodForLogic.managedMemberIds) existingHodForLogic.managedMemberIds = [];
        if (!existingHodForLogic.managedMemberIds.includes(hodId)) {
          existingHodForLogic.managedMemberIds.push(hodId);
          await existingHodForLogic.save();
          console.log('✅ Also added demoted HOD to existing HOD managedMemberIds');
        }
      } else {
        console.log('❌ Not adding to existing HOD (either doesn\'t exist or same as specified manager)');
      }
    }
    ...
    console.log('\n🎉 HOD to Member (Both Relations) conversion successful!');
