...
const { sendTaskCommentEvent, sendNotificationToUser, sendUnreadCountUpdate } = require('../websocket');
...
const populateTaskQuery = (q) => q
  ...
  .populate('assignedToList', 'name email role')
  .populate('assignedBy', 'name email role')
  .populate('companyId', 'name')
  .populate('departmentId', 'name');

const COMMENT_AUTHOR_FIELDS = 'name email role avatar';

const formatComment = (comment) => {
  if (!comment) return null;
  const author = comment.author && typeof comment.author === 'object'
    ? {
      id: comment.author._id || comment.author.id || comment.author,
...
const findTaskWithComments = (taskId) => (
  Task.findById(taskId)
    .select('comments assignedTo assignedToList assignedBy companyId')
    .populate('comments.author', COMMENT_AUTHOR_FIELDS)
);

const canModifyComment = (user, commentAuthorId, task) => {
  if (!user) return false;
  const privilegedRoles = ['super_admin', 'admin', 'hr', 'hr_manager'];
  if (privilegedRoles.includes(user.role)) return true;
  if (commentAuthorId && user.id && String(commentAuthorId) === String(user.id)) return true;
  const isAssignee = task && (
    String(task.assignedBy) === String(user.id) ||
    String(task.assignedTo) === String(user.id) ||
    (Array.isArray(task.assignedToList) && task.assignedToList.some(a => String(a) === String(user.id)))
  );
  if (isAssignee) {
    return true;
  }
...
const sendCommentEvent = (taskId, event, payload) => {
  if (!taskId || !sendTaskCommentEvent) return;
  try {
    sendTaskCommentEvent(taskId, { type: event, data: payload });
  } catch (error) {
    console.warn('WebSocket task comment event failed:', error.message);
  }
};
...
router.get('/:id/comments', async (req, res) => {
  try {
    ...
    const task = await findTaskWithComments(id);
    if (!task) return res.status(404).json({ error: 'Task not found' });
    ...
    return res.status(500).json({ error: 'Failed to load task comments' });
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (parentId && !task.comments.id(parentId)) {
        return res.status(400).json({ error: 'Parent comment not found on this task' });
      }
      ...
      const refreshed = await findTaskWithComments(id);
      const saved = refreshed.comments.find(c => String(c._id) === String(newComment._id));
      const formatted = formatComment(saved);
      sendCommentEvent(id, 'created', formatted);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (!canModifyComment(req.user, comment.author, task)) {
      ...
      const refreshed = await findTaskWithComments(id);
      const updated = refreshed.comments.id(commentId);
      const formatted = formatComment(updated);
      sendCommentEvent(id, 'updated', formatted);
...
router.delete('/:id/comments/:commentId', async (req, res) => {
  try {
    ...
    const collectChildren = (parent) => {
      task.comments.forEach(child => {
        if (child.parentId && String(child.parentId) === parent && !idsToRemove.has(String(child._id))) {
          idsToRemove.add(String(child._id));
          collectChildren(String(child._id));
        }
      });
    };
    collectChildren(String(commentId));

    task.comments = task.comments.filter(c => !idsToRemove.has(String(c._id)));

    await task.save();
    sendCommentEvent(id, 'deleted', { ids: Array.from(idsToRemove) });

    return res.json({ success: true, data: { deletedIds: Array.from(idsToRemove) } });
  } catch (error) {
...
router.post('/', upload.array('files'), validateTaskCreation, auditMiddleware.taskCreated, async (req, res) => {
  try {
    ...
    const { title, description, assignedTo, assignedToList, priority, status = 'assigned', dueDate, departmentId, assigneeType, assignedToRole } = req.body;
    const assignedBy = req.user.id;

    const creator = await User.findById(assignedBy);
    if (!creator) return res.status(404).json({ error: 'Creator not found' });

    // Normalize multiple assignees (allow comma-separated string or array)
    let assigneeIds = [];
    if (assignedTo) assigneeIds.push(assignedTo);
    if (assignedToList && Array.isArray(assignedToList)) assigneeIds.push(...assignedToList);
    if (assignedToList && typeof assignedToList === 'string') {
      assigneeIds.push(...String(assignedToList).split(',').map(s => s.trim()).filter(Boolean));
    }
    assigneeIds = assigneeIds.filter(Boolean).map(id => id.toString());
    assigneeIds = [...new Set(assigneeIds)];

    if (assigneeIds.length === 0) {
      return res.status(400).json({ error: 'Assigned user(s) are required' });
    }

    const validAssignees = await User.find({ _id: { $in: assigneeIds } }).select('_id');
    ...
    if (req.files && Array.isArray(req.files) && req.files.length > 0) {
      ...
      for (const f of req.files) {
        try {
          ...
          // Use resource_type: 'auto' so Cloudinary correctly handles image/video/raw (pdf, docx, xlsx, zip, etc.)
          const uploadOptions = { resource_type: 'auto', folder };
          console.log('Uploading file to Cloudinary:', f.originalname, 'mime=', mime, 'options=', uploadOptions);
          const result = await uploadBufferToCloudinary(f.buffer, uploadOptions);
          console.log('Uploaded to Cloudinary:', result.public_id, 'format=', result.format, 'resource_type=', result.resource_type);
          // fallback format from mimetype
          const fileFormat = result.format || (mime.split('/')[1] || 'N/A');
          task.attachments.push({ name: f.originalname, url: result.secure_url || result.url, size: f.size, type: f.mimetype, format: fileFormat, resourceType: result.resource_type, uploadedBy: req.user.id, uploadedAt: new Date() });
        } catch (uploadErr) {
    ...
    const hydratedTask = await populateTaskQuery(Task.findById(task._id));

    // Create notification(s) if task is assigned to someone
    const notificationRecipients = Array.isArray(task.assignedToList) && task.assignedToList.length
      ? task.assignedToList
    ...
    if (notificationRecipients.length) {
      try {
        for (const recipient of notificationRecipients) {
          ...
          console.log('👤 Assigned to:', recipient);
    ...
    if (notificationRecipients.length && task.priority === 'urgent') {
      try {
        console.log('📧 Sending urgent task assignment email');
        console.log('🚨 Priority:', task.priority);
        console.log('👤 Assigned to:', notificationRecipients);
        ...
        for (const recipient of notificationRecipients) {
          const assignee = await User.findById(recipient).select('firstName lastName email');
          if (!assignee || !assignee.email) {
            console.log('⚠️ Assignee not found or no email for urgent task notification', recipient);
            continue;
          }
          ...
          if (emailResult.success) {
            console.log('✅ Urgent task assignment email sent to:', assignee.email);
          } else {
            console.log('⚠️ Urgent task assignment email failed to send to:', assignee.email, emailResult.error);
          }
        }
      } catch (emailError) {
        console.error('❌ Failed to send urgent task assignment email:', emailError);
        // Don't fail the task creation if email sending fails
      }
    }

    res.status(201).json({ success: true, data: hydratedTask || task });
  } catch (error) {
...
router.post('/:id/assign', validateTaskAssignment, auditMiddleware.taskAssigned, async (req, res) => {
  try {
    ...
    if (resolvedUserId && String(resolvedUserId) !== String(req.user.id)) {
      try {
        const notification = new Notification({
        ...
        sendNotificationToUser(resolvedUserId, {
          id: notification._id,
          title: notification.title,
          message: notification.message,
          type: notification.type,
        ...
        const unreadCount = await Notification.getUnreadCount(resolvedUserId);
        sendUnreadCountUpdate(resolvedUserId, unreadCount);
      } catch (notifErr) {
...
router.put('/:id', [
  ...
  try {
    ...
    if (req.body.assignedTo && req.body.assignedTo !== (task.assignedTo && task.assignedTo.toString())) {
      ...
      task.assignedToList = [u._id];
    }

    // handle reassignment (multiple)
    if (req.body.assignedToList && Array.isArray(req.body.assignedToList)) {
      const ids = req.body.assignedToList.map((id) => id && id.toString()).filter(Boolean);
      if (ids.length) {
        ...
        task.assignmentHistory = task.assignmentHistory || [];
        task.assignmentHistory.push({ from: task.assignedTo || null, to: ids[0], by: req.user.id, byRole: req.user.role, at: new Date(), note: 'Reassigned (multi) via update' });
        task.assignedToList = ids;
        task.assignedTo = ids[0];
      }
    }
...
router.post('/:id/attachments', upload.single('file'), async (req, res) => {
  try {
    ...
    // Use auto resource type so Cloudinary detects image/video/raw correctly for all formats
    ...
    const uploadStream = cloudinary.uploader.upload_stream(uploadOptions, async (error, result) => {
      ...
      console.log('Cloudinary upload succeeded:', { public_id: result.public_id, url: result.secure_url || result.url, resource_type: result.resource_type });
